...
interface SupplierImportResolutionModalProps {
    previewData: any;
    onClose: () => void;
...
export default function SupplierImportResolutionModal({ previewData, onClose, onCommit, onUpdatePreviewData }: SupplierImportResolutionModalProps) {
    const [suppliers, setSuppliers] = useState<any[]>(previewData.suppliers || []);
    ...
    const getFieldLabel = (field: string) => {
        switch (field) {
            case "SupplierName": return "Tên thương hiệu";
            case "Logo": return "Đường dẫn Logo";
    ...
    const revalidateData = (updatedSuppliers: any[]) => {
        const newErrors: any[] = [];
        ...
        updatedSuppliers.forEach((sup) => {
            ...
            if (!sup.supplierName || sup.supplierName.trim() === "") {
                isValid = false;
                newErrors.push({
                    sheet: "Suppliers",
                    row: sup.excelRow,
            ...
            if (sup.supplierName && sup.supplierName.trim() !== "") {
                if (seenNames.has(sup.supplierName.toLowerCase())) {
                    isValid = false;
                    newErrors.push({
                        sheet: "Suppliers",
                        row: sup.excelRow,
                        field: "SupplierName",
                        message: `Tên thương hiệu '${sup.supplierName}' bị trùng lặp trong file Excel`
                    });
                } else {
                    seenNames.add(sup.supplierName.toLowerCase());
                }
            }
    ...
    const saveEdit = (idx: number, err: any) => {
        ...
        let updatedSuppliers = suppliers.map(sup => {
            if (sup.excelRow === err.row) {
                return { ...sup, [fieldName]: editValue };
            }
            return sup;
        });
        ...
        const newErrors = revalidateData(updatedSuppliers);
    ...
    const handleCommit = () => {
        const finalData = {
            ...
            suppliers: suppliers.filter((s: any) => s.isValid)
        };
        onCommit(finalData);
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
                <header className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                    <div>
                        <h2 className="text-2xl font-bold text-primary">Giải Quyết Lỗi &amp; Trùng Lặp Thương Hiệu</h2>
                                ...
                                <table className="w-full text-left bg-rose-50/30 table-fixed">
                                    ...
                                    <tbody>
                                        {errors.map((err: any, idx: number) => {
                                            const item = suppliers.find(s => s.excelRow === err.row);
                                            const fieldName = getFieldNameInJs(err.field);
                        ...
                        <section>
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-bold text-amber-600 flex items-center gap-2">
                                    ...
                                    Xử lý trùng lặp thương hiệu ({duplicates.length})
                                </h3>
                                ...
                                <table className="w-full text-left bg-amber-50/30">
                                    <thead className="bg-amber-100 text-amber-800 text-sm">
                                        <tr>
                                            ...
                                            <th className="p-3">Tên thương hiệu trùng</th>
                ...
                <footer className="px-8 py-6 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
                    <p className="text-sm text-slate-500 font-medium">
                        Thương hiệu hợp lệ sẽ import: <span className="text-secondary font-bold">{suppliers.filter((s: any) => s.isValid).length}</span>
                    </p>
