...
export function LoyaltySection({ token }: LoyaltySectionProps) {
  ...
  const getTransactionBadge = (type: string) => {
    switch (type?.toUpperCase()) {
      ...
      case "PENDING_EARN":
        return <span className="px-2 py-0.5 rounded-[6px] text-[10px] font-bold bg-amber-50 text-amber-600 border border-amber-100">Chờ tích điểm</span>;
      case "CANCELLED_PENDING":
        return <span className="px-2 py-0.5 rounded-[6px] text-[10px] font-bold bg-slate-100 text-slate-500 border border-slate-200">Hủy xu tạm giữ</span>;
      case "SPEND":
      ...
      case "LUCKY_WHEEL":
        return <span className="px-2 py-0.5 rounded-[6px] text-[10px] font-bold bg-fuchsia-50 text-fuchsia-600 border border-fuchsia-100">Vòng quay may mắn</span>;
      default:
    ...
    <div className="bg-white rounded-[16px] shadow-sm border border-slate-100 overflow-hidden divide-y divide-slate-100">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 p-5 md:p-6 bg-slate-50/50">
        ...
        >
          ...
          <div className={`grid grid-cols-3 gap-3 mt-5 pt-3 border-t ${styles.dividerColor || "border-white/10"}`}>
            <div>
              <p className={`text-[10px] uppercase tracking-wider ${styles.subTextColor}`}>Điểm khả dụng</p>
              <p className="text-xl font-black tracking-tight flex items-baseline gap-1">
                {profile?.availablePoints.toLocaleString("vi-VN")}
                <span className={`text-[10px] font-bold ${styles.subTextColor}`}>xu</span>
              </p>
            </div>
            <div>
              <p className={`text-[10px] uppercase tracking-wider opacity-80 flex items-center gap-1`}>
                <Clock size={11} /> Xu giam (7 ngày)
              </p>
              <p className="text-xl font-black tracking-tight text-amber-300">
                +{(profile?.pendingPoints || 0).toLocaleString("vi-VN")} <span className="text-[10px] opacity-80">xu</span>
              </p>
            </div>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="flex items-center bg-slate-50/50 p-0.5 rounded-[6px] border border-slate-200/60 text-[11px] font-bold text-slate-400 overflow-x-auto">
              {(
                [
                  { id: "ALL", label: "Tất cả" },
                  { id: "PENDING_EARN", label: "Chờ tích điểm" },
