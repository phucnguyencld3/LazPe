      ...
      <div className="bg-white p-5 rounded-[12px] flex justify-between items-center shadow-sm border border-slate-100">
        <label className="flex items-center gap-sm cursor-pointer group">
          <input
            ...
            className="w-5 h-5 rounded border-slate-300 text-slate-900 focus:ring-primary/20 accent-rose-500 transition-all cursor-pointer"
          />
        ...
        <div className="flex gap-4">
          <button
            ...
            className="flex items-center gap-1.5 text-slate-500 hover:text-slate-900 transition-colors font-bold text-xs"
          >
            <Trash2 size={14} />
            Xóa mục đã chọn
          </button>
          <button
            ...
            className="flex items-center gap-1.5 text-slate-400 hover:text-slate-900 transition-colors font-bold text-xs border-l border-slate-200 pl-4"
          >
            Xóa sạch giỏ hàng
          </button>
        </div>
          ...
          return (
            <>
              {nonGifts.map((detail) => {
                ...
                return (
                  <div key={detail.cartDetailID} className="flex flex-col relative">
                    {/* Main Product */}
                    <div
                      className={`bg-white p-2.5 sm:p-3 rounded-[12px] shadow-sm flex flex-row gap-2.5 sm:gap-3 items-start sm:items-center group transition-all border relative z-10 ${isChecked
                        ? "border-primary/20 bg-primary/[0.02]"
                        : "border-slate-100 hover:border-slate-200"
                        }`}
                    >
                      <div className="flex items-center h-14 sm:h-16 shrink-0">
                        <input
                          type="checkbox"
                          checked={isChecked}
                          onChange={() => handleToggleCheck(detail.cartDetailID, associatedGift?.cartDetailID)}
                          className="w-4 h-4 rounded border-slate-300 text-slate-900 focus:ring-primary/20 accent-rose-500 transition-all cursor-pointer"
                        />
                      </div>

                      {/* Product Image */}
                      <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-lg overflow-hidden bg-slate-100 border border-slate-100 shrink-0 relative">
                        {image ? (
                          <img
                            ...
                            onError={(e) => {
                              if (!e.currentTarget.dataset.fallbackApplied) {
                                ...
                                e.currentTarget.src = detail.product?.imageUrl || "/images/placeholder.jpg";
                      ...
                      <div className="flex-grow flex flex-col sm:flex-row sm:items-center justify-between gap-3 min-w-0 w-full">
                        ...
                        <div className="flex-grow space-y-1 text-left min-w-0">
                          <h3 className="font-bold text-xs sm:text-sm text-slate-800 hover:text-slate-900 transition-colors line-clamp-2 leading-snug">
                            {isBundle ? (
                              name
                            ) : (
                          ...
                          <div className="flex flex-wrap items-center justify-start gap-1.5">
                            <p className="text-on-surface-variant text-[9px] sm:text-[10px] font-semibold bg-slate-50 border border-slate-100 px-1.5 py-0.5 rounded inline-block">
                              {subtext}
                            </p>
                            {flashSaleItem && (
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                            ...
                            {isQtyExceeded && flashSaleItem && (
                              <div className="text-[9px] text-slate-900 font-bold flex items-center justify-start gap-1 bg-primary/5 border border-primary/20 rounded-md px-1.5 py-0.5 w-fit">
                                <AlertCircle size={10} />
                                Vượt số lượng FS!
                              </div>
                            )}
                        ...
                        <div className="flex items-center gap-3 shrink-0 justify-between sm:justify-start w-full sm:w-auto mt-1 sm:mt-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-dashed border-slate-100">
                          <div className="flex items-center bg-slate-100 rounded-[6px] p-0.5 border border-slate-200">
                            <button
                              ...
                              className="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-[6px] bg-white hover:bg-slate-50 border border-slate-200 shadow-sm text-slate-900 transition-all active:scale-90"
                            >
                              <Minus size={10} />
                            </button>
                            <span className="w-6 sm:w-8 text-center font-bold text-slate-800 text-xs">
                              {detail.quantity}
                            </span>
                            <button
                              onClick={() => handleUpdateQuantity(detail, detail.quantity + 1)}
                              disabled={flashSaleItem && detail.quantity >= maxAllowedQuantity}
                              className="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-[6px] bg-white hover:bg-slate-50 border border-slate-200 shadow-sm text-slate-900 transition-all active:scale-90 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              <Plus size={10} />
                            </button>
                          </div>
                          ...
                          <button
                            ...
                            className="material-symbols-outlined text-slate-400 hover:text-slate-900 p-1.5 hover:bg-primary/5 rounded-[6px] transition-all active:scale-90 text-[18px]"
                          >
                            delete
                          </button>
                        </div>
                            ...
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-md overflow-hidden bg-white border border-slate-100 shrink-0 relative">
                              {associatedGift.variant?.imageUrl || associatedGift.product?.imageUrl ? (
                                <img
                                  className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                                  alt="Ảnh quà tặng"
                                  src={associatedGift.variant?.imageUrl || associatedGift.product?.imageUrl || "/images/placeholder.jpg"}
                                  onError={(e) => {
                                    if (!e.currentTarget.dataset.fallbackApplied) {
                                      e.currentTarget.dataset.fallbackApplied = "true";
                                      e.currentTarget.src = associatedGift.product?.imageUrl || "/images/placeholder.jpg";
                                    } else if (!e.currentTarget.dataset.placeholderApplied) {
                                      e.currentTarget.dataset.placeholderApplied = "true";
                                      e.currentTarget.src = "/images/placeholder.jpg";
                                    }
              ...
              {remainingGifts.map((gift) => {
                ...
                return (
                  <div
                    ...
                    className="bg-emerald-50/[0.3] p-2.5 sm:p-3 rounded-[12px] shadow-sm flex flex-row gap-2.5 sm:gap-3 items-start sm:items-center group transition-all border border-emerald-200"
                  >
                    <div className="flex items-center h-14 sm:h-16 shrink-0">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4 rounded border-emerald-300 text-emerald-500 focus:ring-emerald-500/20 accent-emerald-500 transition-all cursor-pointer"
                      />
                    </div>

                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-lg overflow-hidden bg-white border border-slate-100 shrink-0 relative">
                      {image ? (
                        <img 
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" 
                          alt="Ảnh sản phẩm" 
                          ...
                          onError={(e) => {
                            if (!e.currentTarget.dataset.fallbackApplied) {
                              e.currentTarget.dataset.fallbackApplied = "true";
                              e.currentTarget.src = gift.product?.imageUrl || "/images/placeholder.jpg";
                            } else if (!e.currentTarget.dataset.placeholderApplied) {
                              e.currentTarget.dataset.placeholderApplied = "true";
                              e.currentTarget.src = "/images/placeholder.jpg";
                            }
                    ...
                    <div className="flex-grow flex flex-col sm:flex-row sm:items-center justify-between gap-3 min-w-0 w-full">
                      <div className="flex-grow space-y-1 text-left min-w-0">
                        <h3 className="font-bold text-xs sm:text-sm text-slate-800 hover:text-emerald-600 transition-colors line-clamp-2 leading-snug">
                          {name}
                        </h3>
                        <div className="flex flex-wrap items-center justify-start gap-1.5">
                          <p className="text-on-surface-variant text-[9px] sm:text-[10px] font-semibold bg-white border border-slate-100 px-1.5 py-0.5 rounded inline-block">
                            {subtext}
                          </p>
                          <span className="inline-flex items-center gap-0.5 text-[8px] font-black text-emerald-700 bg-emerald-100 px-1.5 py-0.5 rounded shadow-sm">
                            QUÀ TẶNG
                      ...
                      <div className="flex items-center shrink-0 w-full sm:w-auto mt-1 sm:mt-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-dashed border-emerald-200 justify-end">
                        <span className="px-3 py-1 bg-emerald-100 text-emerald-700 font-bold text-[10px] rounded-[4px] border border-emerald-200">
