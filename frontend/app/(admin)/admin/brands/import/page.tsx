...
import { downloadSupplierTemplate, validateSupplierImport, commitSupplierImport } from "@/lib/features/brands/brandApi";
import SupplierImportResolutionModal from "@/components/admin/brands/SupplierImportResolutionModal";
...
export default function SupplierImportPage() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem("lazpe_supplier_import_draft");
        if (saved) {
            try {
    ...
    useEffect(() => {
        if (previewData) {
            localStorage.setItem("lazpe_supplier_import_draft", JSON.stringify(previewData));
        } else {
            localStorage.removeItem("lazpe_supplier_import_draft");
        }
    }, [previewData]);
    ...
    const handleDownloadTemplate = async () => {
        try {
            ...
            a.href = url;
            a.download = "SupplierImportTemplate.xlsx";
    ...
    const handleValidate = async () => {
        ...
        try {
            ...
            const data = await validateSupplierImport(token, file);
            setPreviewData(data);
    ...
    const commitData = async (dataToCommit: any) => {
        ...
        try {
            const token = localStorage.getItem("token") || sessionStorage.getItem("token") || "";
            await commitSupplierImport(token, {
                suppliers: dataToCommit.suppliers,
                actionableDuplicates: dataToCommit.duplicates
            });
            toast.success("Import thương hiệu thành công!");
            localStorage.removeItem("lazpe_supplier_import_draft");
    ...
    return (
        <main className="w-full pb-20">
            <header className="mb-lg flex items-center justify-between">
                <div>
                    <h1 className="font-headline-md text-headline-md text-primary font-bold">Import Thương Hiệu</h1>
                    <p className="font-body-md text-body-md text-on-surface-variant/70">Tải lên file Excel để thêm nhiều thương hiệu (nhà cung cấp) cùng lúc</p>
                </div>
            ...
            {!previewData && (
                <div className="bg-white p-8 rounded-[8px] shadow-sm border border-slate-100 flex flex-col items-center">
                    <p className="mb-4 text-center text-slate-600">
                        Tải file Excel mẫu để điền dữ liệu thương hiệu:
            ...
            {previewData && (
                <div className="bg-white rounded-[8px] shadow-sm border border-slate-100 overflow-hidden mt-6">
                    <div className="p-4 bg-slate-50 border-b border-slate-200">
                        <h2 className="font-bold text-slate-700 text-lg">Xem trước dữ liệu thương hiệu ({previewData.suppliers.length})</h2>
                    </div>
                    ...
                    <div className="p-6 overflow-x-auto">
                        <div className="overflow-x-auto w-full">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {previewData.suppliers.map((item: any, idx: number) => {
                                        const hasDuplicate = previewData.duplicates.some((d: any) => d.itemCode.toLowerCase() === item.supplierName.toLowerCase());
                                        ...
                                        return (
                                            <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="p-4 text-xs text-slate-500 truncate max-w-[150px]" title={item.logo}>{item.logo || <span className="text-slate-400 italic">Không có</span>}</td>
