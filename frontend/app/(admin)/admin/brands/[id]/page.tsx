...
export default function BrandDetailPage({ params }: BrandDetailPageProps) {
  ...
  return (
    <main className="w-full pb-20 animate-in fade-in duration-300">
      {/* Back to list & Edit Button */}
      <div className="flex items-center justify-between mb-8">
        <Link
          href="/admin/brands"
          className="flex items-center gap-2 text-slate-500 hover:text-slate-800 font-bold text-sm transition-colors cursor-pointer group"
        >
        ...
        {brand && (
          <button
            onClick={() => {
              // Redirect back to main brands page with edit parameters or auto edit trigger
              router.push(`/admin/brands?edit=${brand.supplierID}`);
            }}
            className="bg-primary/10 hover:bg-primary/20 text-primary px-5 py-2 rounded-full font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer"
          >
